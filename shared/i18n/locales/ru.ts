...
export const messages: Messages = {
  ...
  home: {
    title: "LumenLink — передача файлов светом",
    metaDescription:
    ...
    shareSite: "Поделиться LumenLink",
    supportTitle: "Бесплатно, открытый код, без рекламы",
    supportBodyHtml:
      'Если LumenLink вам пригодился, можете <a href="https://buymeacoffee.com/muraliadithkrishnareddy" target="_blank" rel="noopener noreferrer">угостить меня кофе</a>.',
  ...
  send: {
    ...
    shareTitleData: "LumenLink — приёмник",
  ...
  receive: {
    ...
    clearCache: "Очистить кэш LumenLink",
  ...
  verdicts: {
    olderSender: (version) =>
      `Передающий экран использует более старый формат LumenLink (v${version}). Обновите отправляющее устройство.`,
    newerSender: (version) =>
      `Передающий экран использует более новый формат LumenLink (v${version}). Обновите это приложение, чтобы принять поток.`,
    unsupportedFlags:
      "Этот поток использует возможность LumenLink, которую эта версия не умеет читать. Обновите это приложение, чтобы принять поток.",
  },
  ...
  i18n: {
    ...
    switchOffer: "LumenLink доступен на русском.",
