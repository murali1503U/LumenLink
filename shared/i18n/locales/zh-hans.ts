...
export const messages: Messages = {
  ...
  verdicts: {
    olderSender: (version) =>
      `对方屏幕正在以较旧的 LumenLink 格式（v${version}）发送。请更新发送设备。`,
    newerSender: (version) =>
      `对方屏幕正在以较新的 LumenLink 格式（v${version}）发送。请更新本应用后再接收。`,
    unsupportedFlags: "该数据流使用了此版本无法读取的 LumenLink 功能。请更新本应用后再接收。",
  },
